...
function updateExpenseMembers() {
    ...
    members.forEach(member => {
        ...
        div.innerHTML = `<input type="checkbox" id="expenseMember_${member}" value="${member}" checked> <label for="expenseMember_${member}">${member}</label>`;
...
function displayExpenses() {
    ...
    latestExpenses.forEach((expense, index) => {
        ...
        li.innerHTML = `${expense.title} - ¥${expense.amount.toFixed(2)} (支払者: ${expense.payer}, メンバー: ${expense.members.join(', ')}) <button class="btn btn-danger" onclick="removeExpense(${index})">削除</button>`;
...
function displayBackups() {
    ...
    backups.forEach((backup, index) => {
        ...
        div.innerHTML = `
            ...
            <div>
                <button class="btn btn-secondary" onclick="restoreBackup(${index})">復元</button>
                <button class="btn btn-danger" onclick="deleteBackup(${index})">削除</button>
            </div>
        `;
